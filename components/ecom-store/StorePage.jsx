...
//     // now we need a state for cart - so when useer click on cart it should open a cart drwer
//     const [isCartOpen , setIsCartOpen]=useState(false);

//     // now we need a function that will set  -- setIsCartOpen as true when user click on cart
//     function cartOpen(){
//         console.log("cart is clicked : and current value of isCartOpen is : ",isCartOpen);
//         if(isCartOpen){
//             setIsCartOpen(false);
//         }else{
//             setIsCartOpen(true);
...
//     // we need a component for login modal 
//     function LoginModal() {
//         return (
//             <AnimatePresence>
...
//     // now we need a component for cart drwer
//     // function CartDrawer(){
//     //     return(
//     //         <div>
//     //             <h2>Cart Drawer</h2>
...
//                     onClick={() => setIsCartOpen(false)} // close the cart drawer when clicking outside
//                 >
//                     {/* Drawer Panel */}
//                     <motion.div
//                         initial={{ x: "100%" }}
...
//                         className="absolute top-0 right-0 h-full w-[350px] md:w-[420px] bg-white shadow-2xl p-6 overflow-y-auto"
//                         onClick={(e) => e.stopPropagation()} // prevent close on inner click
//                     >
//                         {/* Close Button */}
//                         <button
//                             onClick={() => setIsCartOpen(false)}
//                             className="absolute top-4 right-4 text-slate-500 hover:text-black transition hover:cursor-pointer"
//                         >
//                             <FiX size={28} />
//                         </button>

//                         <h2 className="text-2xl font-bold text-slate-900 mb-8 tracking-wider">
//                             Your Cart
//                         </h2>
...
//                                 <div className="w-20 h-20 bg-slate-200 rounded overflow-hidden">
//                                     <Image
...
//                                     <h3 className="font-semibold text-slate-900 text-base">IMPERIAL VANTAGE</h3>
//                                     <p className="text-sm text-slate-500">₹1,25,000</p>
//                                     <div className="flex items-center gap-4 mt-2">
//                                         <button className="px-2 py-1 border text-base hover:cursor-pointer">-</button>
...
//                             <button className="w-full bg-slate-900 text-white py-3 font-bold uppercase tracking-widest hover:bg-[#D4AF37] transition hover:cursor-pointer">
//                                 Checkout
//                             </button>
//                         </div>
//                     </motion.div>
...
import {
    FiSearch, FiShoppingCart, FiUser, FiChevronDown,
    FiMenu, FiX, FiMail, FiSmartphone, FiHeart, FiMapPin,
    FiTruck, FiHelpCircle, FiArrowRight, FiFilter, FiTrash2,FiPlus,FiMinus
} from 'react-icons/fi';
...
// --- SUB COMPONENTS (Defined OUTSIDE StorePage to prevent re-renders) ---
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 z-[60] flex items-center justify-center p-4 backdrop-blur-sm transition-all">
            <motion.div
                ...
                className="bg-white w-full max-w-md border-t-4 border-[#D4AF37] p-8 relative shadow-2xl rounded-lg md:rounded-none"
            >
                <button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-[#D4AF37] transition hover:cursor-pointer">
                    <FiX size={26} />
                </button>
...
function CartDrawer({ isOpen, onClose, cartItems, onRemove, onUpdateQuantity, total }) {
    return (
        <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        >
            ...
            <motion.div
                ...
                exit={{ x: "100%" }}
                transition={{ type: "tween", duration: 0.4 }}
                className="absolute top-0 right-0 h-full w-[350px] md:w-[450px] bg-white shadow-2xl flex flex-col"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <div className="p-6 border-b flex justify-between items-center">
                    ...
                    <button
                        onClick={onClose}
                        className="text-slate-500 hover:text-black transition hover:cursor-pointer"
                    >
                        <FiX size={28} />
                    </button>
                </div>
                    ...
                    {cartItems.length === 0 ? (
                        ...
                        cartItems.map((item) => (
                            <motion.div
                                layout
                                key={item.id}
                                className="border-b border-slate-100 pb-4 flex gap-4"
                            >
                                <div className="w-20 h-20 bg-slate-100 rounded overflow-hidden relative flex-shrink-0">
                                    <Image
                                        src={item.image}
                                        width={100}
                                        height={100}
                                ...
                                <div className="flex-1 flex flex-col justify-between">
                                    <div>
                                        <div className='flex justify-between items-start'>
                                            <h3 className="font-semibold text-slate-900 text-sm md:text-base pr-4 leading-tight">{item.name}</h3>
                                            <button
                                    ...
                                    <div className="flex justify-between items-end mt-2">
                                        <div className="flex items-center border border-slate-300 rounded">
                                            <button
                                                ...
                                                className="px-3 py-1 hover:bg-slate-100 disabled:opacity-30 disabled:hover:bg-transparent transition cursor-pointer disabled:cursor-not-allowed"
                                            >
                                                <FiMinus size={18} />
                                            </button>
                                            ...
                                            <button
                                                onClick={() => onUpdateQuantity(item.id, 1)}
                                                className="px-3 py-1 hover:bg-slate-100 transition cursor-pointer"
                                            >
                                                <FiPlus size={18} />
                                            </button>
                                        </div>
                ...
                {cartItems.length > 0 && (
                    <div className="p-6 border-t bg-slate-50">
                        ...
                        <button className="w-full bg-slate-900 text-white py-4 font-bold uppercase tracking-widest hover:bg-[#D4AF37] transition hover:cursor-pointer shadow-lg">
                            Checkout Now
                        </button>
                    </div>
                )}
...
export default function StorePage() {
    ...
    function addToCart(product) {
        setCartItems(function (prevItems) {
            let existingItem = prevItems.find(function (item) {
    ...
    const cartItemCount = cartItems.reduce(function (count, item) {
        return count + item.quantity;
    }, 0);

    // Prevent scrolling when overlays are open
    useEffect(function () {
        if (isMobileMenuOpen || showLoginModal || isCartOpen) {
            document.body.style.overflow = 'hidden';
        } else {
            document.body.style.overflow = 'unset';
        }
    }, [isMobileMenuOpen, showLoginModal, isCartOpen]);
                    ...
                    <div className="flex justify-between h-20 md:h-24 items-center gap-4 md:gap-8">
                        ...
                        <div className="flex items-center space-x-4 lg:space-x-8">
                            ...
                            >
                                <FiShoppingCart className="h-6 w-6 text-slate-900 md:text-slate-700 hover:text-[#D4AF37] transition" />
                                {cartItemCount > 0 && (
                                    <span className="absolute -top-2 -right-2 bg-[#D4AF37] text-white text-xs font-bold h-5 w-5 flex items-center justify-center rounded-full animate-bounce-short">
                                        {cartItemCount}
                                    </span>
                                )}
                            </button>
                                    ...
                                    <div className="relative aspect-[3/4] bg-[#F5F5F5] mb-4 cursor-pointer overflow-hidden rounded-md md:rounded-none">
                                        ...
                                        <div className="absolute inset-x-0 bottom-0 md:bottom-4 px-0 md:px-4 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                            ...
                                            <button
                                                ...
                                                className="w-full flex items-center justify-center gap-2 bg-white/90 md:backdrop-blur text-slate-900 py-3 md:py-4 text-sm md:text-base font-bold uppercase tracking-widest hover:bg-[#D4AF37] hover:text-white transition-colors shadow-base hover:cursor-pointer"
                                            >
                                                <FiShoppingCart size={18} />
                                                Add <span className="hidden md:inline">to Cart</span>
                                            </button>
